...
auc_calc <- function(binary_data, continuous_data, subset_name1, subset_name2, subset_name3){   # , temp_results
  ...
  if(temp_nb_positive_pts > 0 & temp_tot_nb_pts > 1 & temp_nb_positive_pts < temp_tot_nb_pts){
    ...
    temp_permut_auc_list <- c()
    for (i in 1:100) {
      
      # Calculate the ROC, return the AUC
      temp_permut_auc <- (roc(binary_data~sample(continuous_data, replace=FALSE))$auc)[1]
      temp_permut_auc_list <- c(temp_permut_auc_list, temp_permut_auc)
      
    }
    
    percentile <- ecdf(temp_permut_auc_list)
    auc_permut_perc <- as.numeric(percentile(temp_auc))
